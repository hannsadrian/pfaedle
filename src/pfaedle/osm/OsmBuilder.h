...
struct BlockSearch : public SearchFunc {
  bool operator()(const Node *n, const StatInfo *si) const {
    if (n->pl().getSI() && n->pl().getSI()->simi(si) < 0.5)
      return true;
...
private:
  ...
  void readNodes(source::OsmSource *source, Graph *g, const RelLst &rels,
  ...
  void readWriteNds(source::OsmSource *source, util::xml::XmlWriter *o,
                    const RelMap &nodeRels, const OsmFilter &filter,
                    const OsmIdSet &bBoxNodes, NIdMap *nodes,
  ...
  void readEdges(source::OsmSource *source, const RelMap &wayRels,
  ...
  void readOrphanStationsWithLocationIndex(
      ...
      const RelLst &rels, const RelMap &nodeRels, const OsmFilter &filter,
      const BBoxIdx &bbox, const AttrKeySet &keepAttrs,
      const FlatRels &flatRels, const OsmReadOpts &opts) const;
  ...
  OsmRel nextRel(source::OsmSource *source, const OsmFilter &filter,
                 const AttrKeySet &keepAttrs) const;
...
protected:
  ...
  getAttrMatchRanked(const DeepAttrLst &rule, osmid id, const AttrMap &attrs,
